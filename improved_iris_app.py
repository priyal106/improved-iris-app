...
classifier=st.sidebar.selectbox('classifier',('SupportVectorMachiene',"LogisticRegression","RandomForestClassifier"))
if st.sidebar.button("predict"):
	if classifier=='SupportVectorMachiene':
		species_type=prediction(svc_model,s_len,s_width,p_len,p_width)
		score=svc_model.score(X_train,y_train)
	elif classifier=='LogisticRegression':
		species_type=prediction(log_reg,s_len,s_width,p_len,p_width)
		score=log_reg.score(X_train,y_train)
	else:
	    species_type=prediction(rf_clf,s_len,s_width,p_len,p_width)
	    score=rf_clf.score(X_train,y_train)
	st.write('Species Predicted:',species_type)
	st.write('Accuracy score of the model:',score)